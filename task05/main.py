...
def generate_color(step, total_steps):
    base_color = np.array([18, 150, 240])
    darken_factor = step / total_steps
    new_color = base_color + darken_factor * (np.array([255, 255, 255]) - base_color)
    new_color = new_color.astype(int)
...
def dfs_visualize(root, total_steps):
    ...
    while stack:
        node = stack.pop()
        if node and node.id not in colors:
